...
# <ol>
#   <li>"server": 실투자 혹은 모의투자 서버 선택 변수 </li>
#   <li>"login_ok": 로그인 상태를 나타내는 불 타입 변수</li>
# </ol>
# "server" 는 실투자 혹은 모의투자 서버 중 선택된 서버를 저장 할 변수이고, "login_ok" 는 로그인 상태를 나타내는 불 타입 변수 입니다.
...
# 모의투자 서버에서 API 사용법을 익히기 위해 MyObjects.server 변수의 값은 'demo' 로 저장했으나, 실투자 서버를 사용을 원할 경우엔 'hts' 로 변수 값을 저장하면 됩니다. 한편, 로그인 결과를 알기위해 MyObjects.login_ok 변수의 기본 값은 'False'로 저장하였고, 로그인 결과 메시지를 XS_event_handler 로 받으면 'True' 로 변경하여 위 코드의 while 문을 빠져 나옵니다.
